...
if [[ -n "$json_output_pe" ]]; then
    ...
    snakemake \
        ...
        --cluster-generic-submit-cmd 'qsub -q slst_pub -N chip_pe.pbs -l nodes=1:ppn=20' \
        --latency-wait 60 \
        --jobs 4 \
        --use-conda \
...
if [[ -n "$json_output_se" ]]; then
    ...
    snakemake \
        -np \
        --executor cluster-generic \
        --cluster-generic-submit-cmd 'qsub -q slst_pub -N chip_se.pbs -l nodes=1:ppn=20' \
        --latency-wait 60 \
        --jobs 4 \
        --use-conda \
        --group-components processing=8 \
...
if [[ -n "$json_output_pe" ]]; then
    ...
    snakemake \
        --executor cluster-generic \
        --cluster-generic-submit-cmd 'qsub -q slst_fat -N chip_pe.pbs -l nodes=1:ppn=10 -j oe' \
        --latency-wait 60 \
        --jobs 5 \
        --use-conda \
        --group-components processing=8 \
...
if [[ -n "$json_output_se" ]]; then
    ...
    snakemake \
        --executor cluster-generic \
        --cluster-generic-submit-cmd 'qsub -q slst_pub -N chip_se.pbs -l nodes=1:ppn=10 -j oe' \
        --latency-wait 60 \
        --jobs 5 \
        --use-conda \
        --group-components processing=8 \
